...
static void win_thread_entry(void* parameter)
{
	...
	struct rtgui_rect rect = {50, 50, 150, 150};

	app = rtgui_app_create("MyApp");
	RT_ASSERT(app != RT_NULL);

    win = rtgui_win_create(RT_NULL, "MyWindow", &rect, RTGUI_WIN_STYLE_DEFAULT | RTGUI_WIN_STYLE_ONTOP);
...
FINSH_FUNCTION_EXPORT(tw, show a top window);
